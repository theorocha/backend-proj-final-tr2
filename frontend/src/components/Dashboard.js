...
const Dashboard = () => {
  ...
  useEffect(() => {
    listarTanques().then(response => {
      const latestTanques = response.data.reduce((acc, tanque) => {
        if (!acc[tanque.id] || new Date(tanque.data) > new Date(acc[tanque.id].data)) {
          acc[tanque.id] = tanque;
        }
        return acc;
      }, {});
      setTanques(Object.values(latestTanques));
    });
  }, []);
          ...
          <Grid item key={tanque.id} xs={12} sm={6} md={4} lg={3}>
            <AnimatedCard>
              <CardContent style={{ textAlign: 'center' }}>
                ...
                <Typography variant="body1">
                  Nível Atual: {tanque.medida_atual}
                </Typography>
